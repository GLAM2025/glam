...
#         )
        ...
        self.short_length = 2
        print(f'using {self.short_length} short length')
    ...
    def train_forward(self, samples, action, inference_params=None):
        ...
        short_batch = torch.empty(
            (sl , (length - sl)*feats.shape[0], self.d_model), 
            device=samples.device)
        
        for i in range(feats.shape[0]):
            index = i * (length -sl)     # 从第五开始
            end = index + length - sl
            for j in range(sl-1):
                short_batch[j, index:end] = feats[i, j+1:length - sl + j + 1]
              
            short_batch[sl-1, index:end] = feats[i, sl:]
        ...
        short_hidden_states =  torch.empty(
            (length - sl, feats.shape[0], self.d_model), 
            device=samples.device)
...
#         latent = all_states + long_hidden_states[:, 4:]
        ...
        # 534 
        long_hidden_states = self.output_proj(hidden_states[:, sl:])
    ...
    def inference_forward(self, samples, action):
        ...
        # 短程处理
        if hidden_states.shape[1] < sl:
            short_hidden_states = hidden_states[:, -1:]
        else:
            short_hidden_states = hidden_states[:, -sl:]
