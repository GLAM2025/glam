...
from sub_models.mamba_model10 import StochasticMamba
...
class WorldModel(nn.Module):
    def __init__(self, in_channels, action_dim,
        ...
        self.delta_categorical_kl_div_loss = CategoricalKLDivLossWithFreeBits(free_bits=0.3)
        self.optimizer = torch.optim.Adam(self.parameters(), lr=1e-4)
        self.scaler = torch.cuda.amp.GradScaler(enabled=self.use_amp)

        print('using mamba_wm_11')
    ...
    def update(self, obs, action, reward, termination, logger=None):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            ...
            delta_loss, delta_real_kl_div = self.delta_categorical_kl_div_loss(delta_post_logits.detach(), delta_logits[:, :-1])
            
            total_loss = reconstruction_loss + reward_loss + termination_loss + 0.5*dynamics_loss + 0.1*representation_loss   + 0.1*delta_loss
