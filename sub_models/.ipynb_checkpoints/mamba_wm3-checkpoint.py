...
class WorldModel(nn.Module):
    ...
    def calc_last_dist_feat(self, latent, action):
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            ...
            dist_feat = self.storm_mamba(latent, action)
    ...
    def predict_next(self, last_flattened_sample, action, log_video=True):
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            dist_feat = self.storm_mamba(last_flattened_sample, action)
...
#         self.hidden_buffer[:, 0:1] = last_dist_feat
        ...
        for i in range(imagine_batch_length):
            ...
            # 旧可变推理方式一，好像错了
            # context_latent = torch.cat((context_latent[:, i+1:], self.latent_buffer[:, :i+1]), dim=1)
            # sample_action = torch.cat((sample_action[:, i+1:], self.action_buffer[:, :i+1]), dim=1)
            ...
            # 新可变推理方式二
            # context_latent = torch.cat((context_latent[:, :], self.latent_buffer[:, i:i+1]), dim=1)
            # sample_action = torch.cat((sample_action[:, :], self.action_buffer[:, i:i+1]), dim=1)      

            # 旧可变推理方式二，好像错了
            # context_latent = torch.cat((context_latent[:, :], self.latent_buffer[:, :i+1]), dim=1)
            # sample_action = torch.cat((sample_action[:, :], self.action_buffer[:, :i+1]), dim=1)
