...
from sub_models.mamba_model9 import StochasticMamba
...
class WorldModel(nn.Module):
    ...
    def update(self, obs, action, reward, termination, logger=None):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            ...
            dist_feat = self.storm_mamba.train_forward(flattened_sample, action)
            prior_logits = self.dist_head.forward_prior(dist_feat)
            ...
            reward_loss = self.symlog_twohot_loss_func(reward_hat, reward[:, 2:])
            termination_loss = self.bce_with_logits_loss_func(termination_hat, termination[:, 2:])
            # dyn-rep loss
            dynamics_loss, dynamics_real_kl_div = self.categorical_kl_div_loss(post_logits[:, 3:].detach(), prior_logits[:, :-1])
            representation_loss, representation_real_kl_div = self.categorical_kl_div_loss(post_logits[:, 3:], prior_logits[:, :-1].detach())
