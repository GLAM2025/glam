...
#         )
    ...
    def train_forward(self, samples, action, inference_params=None):
        ...
        hidden_states = feats
        delta_hidden_states = hidden_states[:, 1:] - hidden_states[:, :-1] # 比hidden length少1
        
        # 长程处理
        for layer in self.longlayers:
            long_hidden_states = layer(
                delta_hidden_states, inference_params=inference_params, 
            ) 
...
#         latent = all_states + long_hidden_states[:, 4:]
        ...
        # 534 
        long_hidden_states = self.output_proj(long_hidden_states[:, 3:])
    ...
    def inference_forward(self, samples, action, inference_params=None):
        '''
        输入一段统一的序列
        ...
        if hidden_states.shape[1] < 2:
            delta_hidden_states = hidden_states
        else:
            delta_hidden_states = hidden_states[:, 1:] - hidden_states[:, :-1] # 比hidden length少1
        ...
        # 534
        long_hidden_states = self.output_proj(long_hidden_states[:, -1:])
