...
from sub_models.mamba_model12 import StochasticMamba
...
class WorldModel(nn.Module):
    def __init__(self, in_channels, action_dim,
        ...
        self.delta_kl_div_loss = CategoricalKLDivLossWithFreeBits(free_bits=0.1)
        lr=1e-4
        print('world model lr:', lr)
    ...
    def update(self, obs, action, reward, termination, logger=None):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            ...
            # total_loss = reconstruction_loss + reward_loss + termination_loss + 0.5*dynamics_loss + 0.1*representation_loss   + 0.1*delta_loss# 7726标准
            total_loss = 1.2*reconstruction_loss + reward_loss + termination_loss + 0.5*dynamics_loss + 0.1*representation_loss   + delta_loss# 7726 bankheist
            # total_loss = reconstruction_loss + 1.8*reward_loss + termination_loss + 0.7*dynamics_loss + 0.2*representation_loss   + 0.1*delta_loss# 7726 breakout
