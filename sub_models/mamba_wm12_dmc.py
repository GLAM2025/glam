...
from sub_models.mamba_model12_dmc import StochasticMamba
...
class WorldModel(nn.Module):
    def __init__(self, in_channels, action_dim,
                 transformer_max_length, transformer_hidden_dim, transformer_num_layers, short_layers, device) -> None:
        super().__init__()
        ...
        self.delta_kl_div_loss = CategoricalKLDivLossWithFreeBits(free_bits=0.3)
    ...
    def calc_last_dist_feat(self, latent, action):
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            ...
            action = rearrange(action.unsqueeze(0), "B D L -> B L D")
            dist_feat = self.storm_mamba.inference_forward(latent, action)
    ...
    def predict_next(self, last_flattened_sample, action, log_video=True):
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            dist_feat = self.storm_mamba.inference_forward(last_flattened_sample, action)
            dist_feat = dist_feat
    ...
    def init_imagine_buffer(self, imagine_batch_size, imagine_batch_length, action_dim, dtype):
        '''
        ...
        if self.imagine_batch_size != imagine_batch_size or self.imagine_batch_length != imagine_batch_length:
            ...
            action_size = (imagine_batch_size, imagine_batch_length, action_dim)
    ...
    def imagine_data(self, agent: agents.ActorCriticAgent, sample_obs, sample_action,
        ...
        self.init_imagine_buffer(imagine_batch_size, imagine_batch_length, sample_action.shape[2], dtype=self.tensor_dtype)
        obs_hat_list = []
        ...
        for i in range(imagine_batch_length):
            ...
            # 新可变推理方式一
            context_latent = torch.cat((context_latent[:, 1:], self.latent_buffer[:, i:i+1]), dim=1)
            sample_action = torch.cat((sample_action[:, 1:], self.action_buffer[:, i:i+1]), dim=1)

            last_obs_hat, last_reward_hat, last_termination_hat, last_latent, last_dist_feat = self.predict_next(
                context_latent, sample_action, log_video=log_video)
    ...
    def update(self, obs, action, reward, termination, logger=None):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            ...
            # encoding for long feat
            delta_embedding = embedding[:, 5:] - embedding[:, 4:-1]
            delta_post_logits = self.delta_dist_head.forward_post(delta_embedding)
            # delta_post_logits = post_logits[:, 5:] - post_logits[:, 4:-1]
            ...
            dist_feat, long_feat = self.storm_mamba.train_forward(flattened_sample, action)
            prior_logits = self.dist_head.forward_prior(dist_feat)

            delta_logits = self.delta_dist_head.forward_prior(long_feat)
            ...
            reward_loss = self.symlog_twohot_loss_func(reward_hat, reward[:, 4:])
            termination_loss = self.bce_with_logits_loss_func(termination_hat, termination[:, 4:])
            # dyn-rep loss
            dynamics_loss, dynamics_real_kl_div = self.categorical_kl_div_loss(post_logits[:, 5:].detach(), prior_logits[:, :-1])
            representation_loss, representation_real_kl_div = self.categorical_kl_div_loss(post_logits[:, 5:], prior_logits[:, :-1].detach())

            delta_loss, delta_real_kl_div = self.delta_kl_div_loss(delta_post_logits.detach(), delta_logits[:, :-1])
            
            total_loss = reconstruction_loss + reward_loss + termination_loss + 0.5*dynamics_loss + 0.1*representation_loss   + 0.1*delta_loss# 7723

            # total_loss = reconstruction_loss + reward_loss + termination_loss + 0.5*dynamics_loss + 0.1*representation_loss   + 0.5*delta_loss# 7722
        ...
        if logger is not None:
            ...
            logger.log("WorldModel/delta_loss", delta_loss.item())
            logger.log("WorldModel/delta_real_kl_div", delta_real_kl_div.item())
