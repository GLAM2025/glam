...
class StochasticTransformerKVCache(nn.Module):
    def __init__(self, stoch_dim, action_dim, feat_dim, num_layers, num_heads, max_length, dropout, device):
        super().__init__()
        self.action_dim = action_dim
    ...
    def reset_kv_cache_list(self, batch_size, dtype):
        ...
        self.kv_cache_list = []
        for layer in self.layer_stack:
            self.kv_cache_list.append(torch.zeros(size=(batch_size, 0, self.feat_dim), dtype=dtype, device=self.device))
