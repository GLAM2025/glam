...
class MetaWorld:
    def __init__(self, name, seed=None, action_repeat=1, size=(64, 64), camera=None, gpu="cuda:0"):
        ...
        self._action_repeat = action_repeat
        self._camera = camera   # /home/hq/CoWorld/Metaworld/metaworld/envs/mujoco/env_dict.py 192
    ...
    def step(self, action):
        ...
        for _ in range(self._action_repeat):
            state, rew, done, timeout, info = self._env.step(action)
            reward += rew or 0.0
            if done or timeout:
                break

        obs = self._env.render()
        return obs, np.array(reward), done, timeout, info
